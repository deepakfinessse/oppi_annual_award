...
public class RegisterValidator : AbstractValidator<RegisterDto>
{
    ...
    {
        RuleFor(x => x.Title).NotEmpty().Must(x => new[] { "Mr", "Miss", "Mrs", "Dr" }.Contains(x)).WithMessage("Title must be Mr, Miss, Mrs, or Dr");
        ...
        RuleFor(x => x.Dob).NotEmpty().Matches(@"^\d{4}-\d{2}-\d{2}$").WithMessage("Date of Birth is required in YYYY-MM-DD format");
        RuleFor(x => x.Gender).NotEmpty().Must(x => new[] { "Male", "Female", "Others", "Other" }.Contains(x)).WithMessage("Gender must be Male or Female");
